...
export default function CartModal({ isOpen, onClose, onSuccess, onOpenSizeChart }) {
  ...
  const handleSubmitOrder = async (e) => {
    ...
    if (!studentId) {
      alert(t.errStudentId || 'กรุณากรอกรหัสนักศึกษา (หรือใส่ - หากเป็นศิษย์เก่า/บุคคลภายนอก)');
      return;
    }
    if (!yearOfStudy) {
      ...
      <div className="bg-white w-full max-w-4xl rounded-2xl sm:rounded-3xl shadow-2xl border border-zinc-200 overflow-hidden flex flex-col max-h-[96vh] sm:max-h-[92vh]">
        ...
        {cartItems.length === 0 ? (
          ...
          <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-5">
            ...
            {isPromoApplied ? (
              <div className="p-3 sm:p-3.5 bg-gradient-to-r from-amber-500/15 via-amber-400/25 to-emerald-500/15 border border-amber-300 rounded-2xl flex items-center justify-between gap-3 text-xs shadow-xs">
                <div className="flex items-center gap-2.5 font-bold text-zinc-900 min-w-0">
                  <span className="text-xl shrink-0">🎉</span>
                  <div className="min-w-0">
                    <div className="truncate">{t.promoAppliedBanner || 'ได้รับโปรโมชั่นพิเศษ: ซื้อ 2 ตัว 399.-'}</div>
                    <div className="text-[11px] text-zinc-600 font-mono font-normal">
                      ({promoPairs} คู่ • {t.saveAmountLabel || 'ประหยัดไป'} <strong className="text-emerald-700 font-bold font-mono">{formatCurrency(discount)}</strong>)
                    </div>
                  </div>
                </div>
              ...
              <div className="p-3 bg-amber-50 border border-amber-200/80 rounded-2xl flex items-center justify-between gap-2 text-xs text-amber-950">
                <div className="flex items-center gap-2">
                  <span className="text-base">💡</span>
                  <span className="font-medium text-[11.5px]">{t.promoHintBanner || 'ซื้อเพิ่มอีก 1 ตัว เพื่อรับสิทธิ์โปรโมชั่น 2 ตัวเพียง 399.-'}</span>
                </div>
                <button
                  type="button"
                  onClick={onClose}
                  className="px-2.5 py-1 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold font-mono text-[10.5px] rounded-lg transition-colors shrink-0"
                >
                  + เพิ่มเสื้ออีกตัว
                </button>
              </div>
            ...
            <form onSubmit={handleSubmitOrder} noValidate className="space-y-6">
              ...
              <div className="space-y-3.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-zinc-700 mb-1">{t.studentIdLabel || 'รหัสนักศึกษา (9 หลัก) *'}</label>
                    <input
                      type="text"
                      required
                      placeholder={t.studentIdPlaceholder || 'เช่น 650610123'}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div>
                    ...
                    >
                      ...
                      {(t.years || STORE_CONFIG.years).map((y) => (
                  ...
                  <div>
                    ...
                    >
                      ...
                      <optgroup label={t.undergradGroup || 'UNDERGRADUATE PROGRAMS'}>
                        {(t.undergraduateMajors || STORE_CONFIG.undergraduateMajors || []).map((m) => (
                          <option key={m} value={m}>{m}</option>
                        ))}
                      </optgroup>
                      <optgroup label={t.gradGroup || 'POSTGRADUATE PROGRAMS'}>
                        {(t.graduateMajors || STORE_CONFIG.graduateMajors || []).map((m) => (
                          <option key={m} value={m}>{m}</option>
                        ))}
                      </optgroup>
                      <optgroup label={t.otherGroup || 'ALUMNI & EXTERNAL'}>
                        {(t.otherMajors || STORE_CONFIG.otherMajors || []).map((m) => (
                          <option key={m} value={m}>{m}</option>
                        ))}
                      </optgroup>
                    </select>
              ...
              <div className="pt-4 border-t border-zinc-200 space-y-3">
                <div className="p-3.5 bg-zinc-50 rounded-xl border border-zinc-200 space-y-2 text-xs font-mono">
                  <div className="flex justify-between text-zinc-600">
                    <span>{t.regularSubtotalLabel || 'ยอดรวมสินค้า'} ({totalItems} ตัว):</span>
                    <span className={isPromoApplied ? 'line-through text-zinc-400' : 'font-bold text-zinc-800'}>
                      {formatCurrency(rawSubtotal || subtotal)}
                    </span>
                  </div>
                  ...
                  {isPromoApplied && (
                    <div className="flex justify-between items-center text-emerald-600 font-bold bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-200/60">
                      <span className="flex items-center gap-1">
                        <span>⚡</span>
                        <span>{t.promoDiscountLabel || 'ส่วนลดโปรโมชั่น (2 ตัว 399.-):'}</span>
                      </span>
                  ...
                  {isPromoApplied && (
                    <div className="flex justify-between text-zinc-700 font-bold pt-0.5">
                      <span>ยอดหลังหักส่วนลด:</span>
                      <span className="text-zinc-950">{formatCurrency(subtotal)}</span>
                    </div>
                  )}

                  {deliveryMethod === 'shipping' && (
                    <div className="flex justify-between text-zinc-600">
                      <span>{t.shippingOption || 'ค่าบริการจัดส่งพัสดุ'}:</span>
                  ...
                  <div className="flex justify-between text-sm font-bold text-zinc-950 pt-2 border-t border-zinc-200">
                    <span>{t.grandTotalLabel || 'ยอดชำระสุทธิ (Grand Total)'}:</span>
