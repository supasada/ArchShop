...
export function CartProvider({ children }) {
    ...
    setCartItems((prevItems) => {
      ...
      if (existingIndex > -1) {
        ...
        const newItem = {
          ...
          price: Number(product.price) || 219,
          size: chosenSize,
          color: chosenColor,
          quantity: qty,
  ...
  // Calculate promotion: Buy 2 shirts for 399 THB
  const allUnitPrices = [];
  cartItems.forEach((item) => {
    const qty = Math.max(1, parseInt(item.quantity, 10) || 1);
    const price = Number(item.price) || 219;
    for (let i = 0; i < qty; i++) {
      allUnitPrices.push(price);
    }
  });

  // Sort descending so the most expensive pairs get bundled first for customer's best savings
  allUnitPrices.sort((a, b) => b - a);

  const rawSubtotal = allUnitPrices.reduce((sum, p) => sum + p, 0);
  const promoPairs = Math.floor(totalItems / 2);
  const isPromoApplied = promoPairs > 0;

  let promoSubtotal = 0;
  // Apply 399 THB for each pair
  for (let i = 0; i < promoPairs * 2; i += 2) {
    promoSubtotal += 399;
  }
  // Add remaining non-paired single shirts at regular price
  for (let i = promoPairs * 2; i < allUnitPrices.length; i++) {
    promoSubtotal += allUnitPrices[i];
  }

  const discount = Math.max(0, rawSubtotal - promoSubtotal);
  const subtotal = isPromoApplied ? promoSubtotal : rawSubtotal;
